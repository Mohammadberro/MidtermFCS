...
class Browser:
    ...
    def open_tab(self, Title, URL, isParent=True):
        ...
        if isParent:
            ...
            self.tabs_dict_list.append(new_tab)
    ...
    def close_tab(self, index):
        try:
            index = int(index)
            tab_title = self.tabs[index].title
            self.tabs.pop(index)
        except IndexError:
    ...
    def switch_tab(self, index=-1):
        desired_url = self.tabs[index].url
        ...
        try:
            ...
            if input(f"Do you want to change the url : {desired_url} y/n") == "y":
                self.tabs[index].url = input(f"Enter the new url for {self.tabs[index].title}")
        ...
        self.tabs[index].web_source = website_html
        print(website_html)

    def display_all_tabs(self):
        for n in range(0, len(self.tabs)):
            print(f"{n + 1}.\t{self.tabs[n].title}")
            if self.tabs[n].nested_tabs:
                for i in range(0, len(self.tabs[n].nested_tabs)):
                    print(f"\t{n + 1}.{i + 1}\t{self.tabs[n].nested_tabs[i].title}")
    ...
    def open_nested_tab(self, indicator):
        ...
        if indicator.isdigit():
            parent_tab = self.tabs[int(indicator)]
            try:
